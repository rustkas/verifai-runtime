use crate::bytes::{push_bytes, push_u16_le, Reader, BytesError};
...
impl ProofArtifactV0 {
    ...
    pub fn verify_signature(&self) -> Result<(), BytesError> {
        let vk = VerifyingKey::from_bytes(&self.sig_pubkey).map_err(|_| BytesError::InvalidLength)?;
        let sig = Signature::from_bytes(&self.signature);
        let msg = self.message_to_sign();
        vk.verify(&msg, &sig).map_err(|_| BytesError::InvalidLength)?;
        Ok(())
    }
